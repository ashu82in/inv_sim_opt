...
with tab3:
    ...
    # --- 2. ADVANCED OPTIMIZER TUNING (Hidden Slider) ---
    with st.expander("⚙️ Advanced Optimizer Tuning"):
        st.info("Precision vs. Speed: Higher scenarios increase accuracy but slow down execution.")
        n_opt_sim = st.select_slider(
            "Simulation Scenarios (Precision)", 
            options=[100, 500, 1000, 2000, 5000], 
            value=1000
        )
        num_pop = st.slider("Population Size (Diversity)", 20, 100, 40)
        max_gen = st.slider("Max Generations (Safety Cap)", 20, 300, 100)
    ...
    if st.button("🚀 Run Adaptive Optimization"):
        ...
        avg_ltd = avg_demand * lead_time
        sigma_ltd = std_demand * np.sqrt(lead_time)
        rop_f = int(max(0, avg_ltd - (2.5 * sigma_ltd)))
        rop_c = int(avg_ltd + (8.0 * sigma_ltd))
        ...
        for gen in range(max_gen):
            ...
            for r_t, q_t in pop:
                # --- TURBO VECTORIZED CORE SIMULATION ---
                inv = np.full(n_opt_sim, opening_balance, dtype=float)
                # Lead time buffer to prevent index errors in the arrivals matrix
                ...
                avg_cost_val = (h_costs + (orders * ordering_cost)).mean()
                
                # Penalty Engine
                penalty = 0
                if use_so_constraint and so_p99 > target_so_days: penalty += (so_p99 - target_so_days) * 30000
                if use_wc_constraint and wc_p99 > max_wc_allowed: penalty += (wc_p99 - max_wc_allowed) * 150
                if fr_p1 < target_fr: penalty += (target_fr - fr_p1) * 45000
                
                fitness_scores.append(avg_cost_val + penalty)
                gen_metrics.append({'cost': avg_cost_val, 'fr': fr_p1, 'so': so_p99, 'wc': wc_p99})
            ...
            log_data.append({
                ...
                "ROP": pop[best_idx][0], 
                "Qty": pop[best_idx][1], 
                "Avg Cost": round(gen_metrics[best_idx]['cost'], 2),
                "SO Days (P99)": gen_metrics[best_idx]['so']
            })
            
            status_text.text(f"Generation {gen+1}: Finding Optimal Balance... (Patience: {gens_without_imp}/{patience})")
            progress_bar.progress((gen + 1) / max_gen)

            if gens_without_imp >= patience:
                status_text.success(f"✅ AI has converged on the most efficient policy at Gen {gen+1}!")
                break

            # Evolution Logic (Elitism + Mutation)
            ranked_indices = np.argsort(fitness_scores)
            ranked_pop = [pop[i] for i in ranked_indices]
            new_pop = ranked_pop[:4] 
            while len(new_pop) < num_pop:
                parents = random.sample(ranked_pop[:12], 2)
                child = [int((parents[0][0]+parents[1][0])/2), int((parents[0][1]+parents[1][1])/2)]
                if random.random() < 0.3:
                    child[0] = np.clip(child[0] + random.randint(-25,25), rop_f, rop_c)
                new_pop.append(child)
            pop = new_pop

        # --- 4. FINAL KPI & VISUAL DASHBOARD ---
        ...
        k1.metric("Optimal Strategy (ROP/Q)", f"{pop[0][0]} / {pop[0][1]}")
        k2.metric("Min Fill Rate (P1)", f"{best_m['fr']:.2f}%", 
                  delta="PASS ✅" if fr_pass else "FAIL ❌", delta_color="normal" if fr_pass else "inverse")
        k3.metric("Stockout Risk (P99)", f"{best_m['so']:.1f} Days", 
                  delta="PASS ✅" if so_pass else "FAIL ❌", delta_color="normal" if so_pass else "inverse")

        # Visualization Tabs
        v_tab1, v_tab2 = st.tabs(["📉 Learning Curve", "📜 Simulation Log"])
        with v_tab1:
            st.plotly_chart(px.line(y=history, title="AI Optimization Progress", labels={'y':'Fitness (Lower is Better)', 'index':'Generation'}), use_container_width=True)
        with v_tab2:
